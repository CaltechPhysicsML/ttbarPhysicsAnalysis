...
def do_analysis(chain, analyze_this, outfile, eventtype, dt_this):
    ...
    for e in range(nentries):
        ...
        if (analyze_this['Electron.PT']):
            ...
            (e_maxpt, e_maxpt_phi, e_maxpt_eta, e_nume) = fill_Electron_hist(chain, leaf, entry, numElectrons, min_ept_per_event, max_ept_per_event, phileaf, etaleaf, lepton_vec)
        ...
        if (analyze_this['MuonTight.PT']):
            ...
            (u_maxpt, u_maxpt_phi, u_maxpt_eta, u_numu) = fill_Muon_hist(chain, leaf, entry, numMuons, min_upt_per_event, max_upt_per_event, phileaf, etaleaf, lepton_vec)
        ...
        if (analyze_this['Jet.PT']):
            ...
            # returns phi of max pt for entry
            (jet_maxpt, jet_maxpt_phi, jet_maxpt_eta, jet_minpt, jet_numjets, jet_ht) = fill_JetPT_hist(chain, leaf, entry, numJets, min_jetpt_per_event, max_jetpt_per_event, HTfill, HT, phileaf, etaleaf, lepton_vec)
        ...
        if (analyze_this['Jet.BTag']):
            ...
            btag_medium = fill_JetBTag_hist(chain, leaf, entry, loose, medium, tight)    
...
def aggregate_main():
    ...
    # get a list of features for decision tree
    dt_features = ['numJets', 'maxjetpt', 'minjetpt', 'medium', 'numLeptons', 'maxlpt', 'MET', 'MT', 'HT', 'DPHI_metlep', 'DPHI_metjet', 'DR_lepjet']
    
    # create a list of lists to prepare data filling
    dt_data1 = np.empty((len(dt_features), 0)).tolist()
    dt_data2 = np.empty((len(dt_features), 0)).tolist()
    dt_data3 = np.empty((len(dt_features), 0)).tolist()
    
    # create a dictionary to modify values of any features when analyzing data
    dt_list = [dict(zip(dt_features, dt_data1)), dict(zip(dt_features, dt_data2)), dict(zip(dt_features, dt_data3))] #* len(allfiles)
    ...
    for i in allfiles:
        ...
        dt_this = dt_list[cntr]
        print dt_this
        print cntr
        
        do_analysis(chain, analyze_this, newf, eventtype, dt_list[cntr])
        
        print len(dt_list[0]['numJets'])
        print len(dt_list[1]['numJets'])
        print len(dt_list[2]['numJets'])
        
        #dt_list[cntr] = dt_this
        
        print len(dt_list[cntr]['numJets'])
        cntr = cntr + 1
    ...
    cPickle.dump(dt_list, output)
    output.close()

    # dt list does tt, then qcd, then wjets in its list
    return dt_list
